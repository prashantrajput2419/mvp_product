<template>
   <div class="main">
      <!-- navbar-->
      <header class="header">
         <nav class="navbar navbar-expand-lg" style="margin: unset !important;">
           ...
           <div class="container-fluid">  
             <!-- Navbar Header  --><router-link :to="{name: 'productView'}" class="navbar-brand"><img src="@/assets/logo-epos.png" style="height: 119px;width: 166px;" /></router-link>
             <button type="button" data-toggle="collapse" data-target="#navbarCollapse" aria-controls="navbarCollapse" aria-expanded="false" aria-label="Toggle navigation" class="navbar-toggler navbar-toggler-right"><i class="fa fa-bars"></i></button>
             <!-- Navbar Collapse -->
             <div id="navbarCollapse" class="collapse navbar-collapse">
               <ul class="navbar-nav mx-auto">
                 <li class="nav-item"><router-link :to="{name: 'productView'}" class="nav-link">Home</router-link></li>
                 <li class="nav-item"><router-link :to="{name: 'orders'}" class="nav-link">Orders </router-link>
                 </li>
               </ul>
               <div class="right-col d-flex align-items-lg-center flex-column flex-lg-row">
                 <div class="cart dropdown show"><a id="cartdetails" href="https://example.com" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" class="dropdown-toggle"><i class="icon-cart"></i>
                     <div class="cart-no" v-if="totalProduct > 0">{{totalProduct}}</div><div class="cart-no" v-if="totalProduct == 0">0</div></a><a href="cart.html" class="text-primary view-cart">View Cart</a>
                   <div aria-labelledby="cartdetails" class="dropdown-menu" v-if="totalProduct > 0">
                     <!-- cart item-->
                     <div class="dropdown-item cart-product" v-for="(cartproduct, index) in cartproducts">
                       <div class="d-flex align-items-center">
                         <div class="img"><img v-bind:src="urlimage + '/product_images/' + cartproduct.images[0].image_name" alt="..." class="img-fluid"></div>
                         <div class="details d-flex justify-content-between">
                           <div class="text"> <a @click="showProduct(cartproduct.id)" style="cursor: pointer;"><strong>{{cartproduct.name}}</strong></a><small>Quantity: {{cartproduct.qty}} </small><span class="price"><i class="fa fa-rupee"></i>{{cartproduct.amount.toFixed(2)}} </span><small>Total Price: <i class="fa fa-rupee"></i>{{cartproduct.amount.toFixed(2) * cartproduct.qty}} </small></div><a @click="deletePro(cartproduct.id)" class="delete"><i class="fa fa-trash-o"></i></a>
                         </div>
                       </div>
                     </div>
                     <!-- total price-->
                     <div class="dropdown-item total-price d-flex justify-content-between"><span>Total</span><strong class="text-primary"><i class="fa fa-rupee"></i>{{totalPrice.reduce((acc, item) => acc + item, 0).toFixed(2)}}</strong></div>
                     <!-- call to actions-->
                     <div class="dropdown-item CTA d-flex"><router-link :to="{name: 'cart'}" class="btn btn-template wide" style="padding-left: unset;padding-right: unset;">View Item List</router-link></div>
                   </div>
                 </div>
               </div>
             </div>
    ...
    if (localStorage.getItem('proId')) {
      ...
      JSON.parse(localStorage.getItem('proId')).forEach((val)=>{
        axios.get(domain.endpoint + 'products/' + val.proId).then(proResponse => {
          this.cartproducts.push(Object.assign(proResponse.data.data, { 'qty': val.qty }))
