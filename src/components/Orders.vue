   ...
   <div class="orders" v-else>
      <section class="hero hero-page gray-bg padding-small">
         <div class="container">
           <div class="row d-flex">
             <div class="col-lg-9 order-2 order-lg-1">
               <h1 style="font-size: 2.5rem;">Your orders</h1><p class="lead">Your orders in one place.</p>
             </div>
           </div>
         </div>
      </section>
               ...
               <table class="table table-hover table-responsive-md">
                 ...
                 <tbody>
                   <tr v-for="order in limitBy(orders, 10, page * 10 - 10)">
                     ...
                     <td><router-link :to="{name: 'ordersdetail', params: {'orderId': order.id}}" class="btn btn-primary btn-sm">View</router-link></td>
                     <td>{{order.status}}</td>
                   </tr>
                 </tbody>
               </table>
...
export default {
  ...
  mounted: function () {
    axios.get(domain.endpoint + 'kitchenorders').then(orderResponse => {
